...
function CommentCard({
  ...
  return (
    <motion.div
      ...
      className="group flex gap-3 p-4 rounded-xl border border-border bg-background-secondary hover:border-text-light/40 transition-colors duration-200"
    >
      {/* Monochrome avatar */}
      <div
        className="flex-shrink-0 w-9 h-9 rounded-full flex items-center justify-center
          bg-background border border-border text-text-secondary font-bold text-sm select-none"
      >
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        ...
        setError(
          isZh ? "提交失败，请稍后再试" : "Failed to submit, please try again"
        );
      }
    });
  };
          ...
          <AnimatePresence>
            {comments.length > 0 && (
              <motion.span
                ...
                animate={{ scale: 1 }}
                exit={{ scale: 0 }}
                className="absolute -top-1.5 -right-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-text-primary text-[9px] font-bold text-background"
              >
                {comments.length}
              </motion.span>
            )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-text-secondary mb-1.5">
              {isZh ? "昵称" : "Name"} <span className="text-red-500">*</span>
            </label>
            <div className="relative">
              ...
              <input
                ...
                className={cn(
                  ...
                  "focus:outline-none focus:ring-2 focus:ring-text-primary/20 focus:border-text-secondary",
